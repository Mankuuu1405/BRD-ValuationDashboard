...
const FieldVerifications = () => {
  ...
  const filtered = verifications.filter(v => {
    if (filter !== 'all' && v.status.toLowerCase() !== filter.toLowerCase()) return false;
    if (searchQuery && !v.property.toLowerCase().includes(searchQuery.toLowerCase()) &&
        !v.id.toLowerCase().includes(searchQuery.toLowerCase()) &&
        !v.address.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Field Verifications</h1>
          <p className="mt-1 text-sm text-gray-500">Track and manage property field verifications</p>
        </div>
        <div className="flex gap-3">
          <button onClick={handleScheduleVerification} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            Schedule Verification
          </button>
          <button onClick={handleAssignAgent} className="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">
            Assign Agent
          </button>
        </div>
      </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filtered.map((verification) => (
                <tr key={verification.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{verification.id}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{verification.property}</td>
                  <td className="px-6 py-4 text-sm text-gray-900">{verification.address}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{verification.owner}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      ...
                      verification.status === 'In Progress' ? 'bg-blue-100 text-blue-800' :
                      verification.status === 'Scheduled' ? 'bg-purple-100 text-purple-800' :
                      'bg-yellow-100 text-yellow-800'
                    }`}>
                      {verification.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{verification.agent}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h3 className="text-lg font-semibold text-blue-900 mb-2">Field Verification Guidelines</h3>

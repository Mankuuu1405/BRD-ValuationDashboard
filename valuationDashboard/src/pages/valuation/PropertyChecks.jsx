...
const PropertyChecks = () => {
  ...
  const [isNewPropertyCheckModalOpen, setIsNewPropertyCheckModalOpen] = useState(false);
  ...
  const filteredProperties = properties.filter(prop => {
    if (filter !== 'all' && prop.status.toLowerCase() !== filter.toLowerCase()) return false;
    if (searchQuery && !prop.name.toLowerCase().includes(searchQuery.toLowerCase()) &&
        !prop.id.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage and track property inspection checklists</p>
        </div>
        <button onClick={() => setIsNewPropertyCheckModalOpen(true)} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
          New Property Check
        </button>
      </div>
            ...
            <div key={property.id} className="border rounded-lg overflow-hidden">
              <div className="p-4 border-b bg-gray-50">
                <div className="flex justify-between items-start">
                  ...
                  <span className={`px-2 py-1 text-xs font-semibold rounded-full bg-${getStatusColor(property.status)}-100 text-${getStatusColor(property.status)}-800`}>
                    {property.status}
                  </span>
                </div>
              ...
              <div className="p-4">
                <h4 className="text-sm font-medium text-gray-900 mb-2">Check Items</h4>
                <ul className="space-y-2">
                  {property.checkItems.map((item) => (
                    <li key={item.id} className="flex items-center justify-between">
                      ...
                      <select
                        value={item.status}
                        onChange={(e) => handleCheckItemUpdate(property.id, item.id, e.target.value)}
                        className={`text-sm px-2 py-1 rounded border bg-${getStatusColor(item.status)}-100 text-${getStatusColor(item.status)}-800`}
                      >
              ...
              <div className="p-4 border-t bg-gray-50">
                <div className="flex justify-between items-center">
                  <div className="text-sm text-gray-500">
                    Assigned to: <span className="text-gray-900">{property.assignedTo}</span>
                  </div>
                  <button 
                    onClick={() => {
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h3 className="text-lg font-semibold text-blue-900 mb-2">Property Check Guidelines</h3>
